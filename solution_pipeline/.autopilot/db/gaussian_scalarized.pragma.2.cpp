# 1 "/users/deastir/mcaceres/Documents/VIVADO/TP_Vivado_HLS/TP_Vivado_HLS/gaussian/solution1/.autopilot/db/gaussian_scalarized.pragma.1.cpp"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 2 "src/gaussian_scalarized.cpp" 2
...
{_ssdm_SpecArrayDimSize(in,1080);_ssdm_SpecArrayDimSize(out,1080);
 ...
 for (i = 1; i < 1080; ++i) {
  ...
  gaussian_scalarized_label0_pipeline:for (j = 2; j < 1920; ++j) {_ssdm_op_SpecLoopName("gaussian_scalarized_label0_pipeline");_ssdm_RegionBegin("gaussian_scalarized_label0_pipeline");
_ssdm_op_SpecPipeline(1, 1, 1, "");
# 24 "src/gaussian_scalarized.cpp"

   tmp2 = k[0] * in[i - 1][j] + k[1] * in[i][j] + k[2] * in[i + 1][j];
   out[i][j - 1] = k[0] * tmp0 + k[1] * tmp1+ k[2] * tmp2;
   tmp0 = tmp1;
   tmp1 = tmp2;
  _ssdm_RegionEnd("gaussian_scalarized_label0_pipeline");}
 }
